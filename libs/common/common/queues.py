...
QUEUE_TOPIC_CONTRACTS: tuple[QueueTopicContract, ...] = (
    ...
    QueueTopicContract(
        pubsub=TITUS_PUBSUB,
        topic=TITUS_INPUT_TOPIC,
        producers=("file_enrichment.activities.plaintext_handler.handle_file_if_plaintext",),
        consumers=("external.titus",),
        required_payload_keys=("object_id", "workflow_id"),
    ),
    QueueTopicContract(
        pubsub=TITUS_PUBSUB,
        topic=TITUS_OUTPUT_TOPIC,
        producers=("external.titus",),
        consumers=("file_enrichment.controller.titus_subscription_handler",),
        required_payload_keys=("object_id",),
    ),
)
